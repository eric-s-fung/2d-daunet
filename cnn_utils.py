...
from tensorflow.keras.layers import Input, Conv2D, MaxPooling2D, concatenate,add
from tensorflow.keras.layers import Conv3D, MaxPooling3D,UpSampling3D,BatchNormalization
from tensorflow.keras.layers import UpSampling2D, Dropout, Activation, Add 
...
def get_unet_mod(patch_size = (None,None),learning_rate = 1e-5,\
    ...
    conv6 = Conv2D(512, kshape, activation='relu', padding='same')(up6)
    conv6 = Conv2D(512, kshape, activation='relu', padding='same')(conv6)

    up7 = concatenate([UpSampling2D(size=(2, 2))(conv6), conv3],axis=-1)
    up7 = Dropout(dropout)(up7)
    conv7 = Conv2D(256, kshape, activation='relu', padding='same')(up7)
    conv7 = Conv2D(256, kshape, activation='relu', padding='same')(conv7)

    up8 = concatenate([UpSampling2D(size=(2, 2))(conv7), conv2],axis=-1)
    up8 = Dropout(dropout)(up8)
    conv8 = Conv2D(128, kshape, activation='relu', padding='same')(up8)
    conv8 = Conv2D(128, kshape, activation='relu', padding='same')(conv8)

    up9 = concatenate([UpSampling2D(size=(2, 2))(conv8), conv1], axis=-1)
    up9 = Dropout(dropout)(up9)
    conv9 = Conv2D(64, kshape, activation='relu', padding='same')(up9)
    conv9 = Conv2D(64, kshape, activation='relu', padding='same')(conv9)
